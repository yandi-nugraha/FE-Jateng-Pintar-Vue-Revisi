      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav ms-auto ml-auto mt-2 nav-item-list" id="nav">
          <li
            ...
            :class="
              $route.name == 'home' ||
              $route.name == 'list-sumber-belajar' ||
              $route.name == 'sumber-belajar' ||
              $route.name == 'detail-sumber-belajar'
                ? 'active'
                : ''
            "
          >
            <router-link to="/" class="nav-link nav-teks px-4"
              >Beranda</router-link
            >
          </li>
          <li class="nav-item" :class="$route.name == 'acara' ? 'active' : ''">
            <router-link to="/acara" class="nav-link nav-teks px-4"
              >Acara</router-link
            >
          </li>
          <li class="nav-item" :class="$route.name == 'kontak' ? 'active' : ''">
            <router-link to="/kontak" class="nav-link nav-teks"
              >Kontak</router-link
            >
          </li>
          <li class="nav-item sign-btn">
            <a class="nav-link" id="navbarDropdown" role="button">
              <span class="sign-btn-teks"
                >Login <img :src="signInBtn" alt="Login Icon"
              /></span>
            </a>
          </li>
        </ul>
...
.nav-teks {
  ...
  background: linear-gradient(0deg, #ee269d 0%, #ee269d 100%),
    linear-gradient(
      200deg,
      rgba(238, 38, 157, 0.8) 0%,
      rgba(238, 38, 157, 1) 100%
    );
